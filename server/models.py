...
class BertFineTuner(pl.LightningModule):
    def __init__(self, hparams):
        ...
        print("~~~~~~ Using Bert ~~~~~~~~~")
        self.hparams = hparams
        config = BertConfig.from_pretrained(hparams.model_name_or_path)
        config.num_labels = 3
        self.model = BertForSequenceClassification.from_pretrained(
            hparams.model_name_or_path, config=config
        )
        self.tokenizer = BertTokenizer.from_pretrained(hparams.tokenizer_name_or_path)
    ...
    def calculate_scores(self, predictions, ground_truths):
        ...
        for i in range(len(predictions)):
            em_score += self.exact_match_score(predictions[i], ground_truths[i])
        return em_score * 100
    ...
    def forward(self, inputs, labels):
        # print("~~~~ FORWARD")
        return self.model(
            input_ids=batch["input"]["input_ids"][0],
            attention_mask=batch["input"]["attention_mask"][0],
            token_type_ids=batch["input"]["token_type_ids"][0],
            labels=batch["label"][0],
        )
        # return self.model(**inputs, labels=labels)

    def _step(self, batch):
        # outputs = self.model(**batch["input"], labels=batch["label"])
        """
        print("~~~~ STEP")
        print(batch['input']['input_ids'][0])
        print(batch['input']['attention_mask'][0])
        print(batch['input']['token_type_ids'][0])
        print(batch['label'][0])
        """
        outputs = self.model(
            input_ids=batch["input"]["input_ids"][0],
    ...
    def _val_step(self, batch):
        ...
        outputs = self(**batch["input"], labels=batch["label"])
        em = self.calculate_scores(outputs, batch["label"])
        self.log("em_score", em, prog_bar=True, logger=True)
    ...
    def train_dataloader(self):
        train_dataset = self.get_dataset(
            self.tokenizer, type_path="train", args=self.hparams
        )
        dataloader = DataLoader(
            train_dataset,
            batch_size=self.hparams.train_batch_size,
    ...
    def val_dataloader(self):
        ...
        print(f"~!~!~!` val dataset: {len(validation_dataset)}")
        dataloader = DataLoader(
            validation_dataset,
            batch_size=self.hparams.eval_batch_size,
            num_workers=self.hparams.num_workers,
